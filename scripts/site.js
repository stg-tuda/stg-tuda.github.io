(function (window, document) {
  ...
  const siteContent = window.STG_SITE_CONTENT || {};
  const newsSource = Array.isArray(window.STG_NEWS)
    ? window.STG_NEWS
    ...
    renderHighlights(newsItems = []) {
      ...
      const archiveLink = Utils.createElement('a', {
        ...
        attrs: {
          ...
          'aria-label': 'Browse the full STG news archive'
        }
      });
  ...
  const PeopleModule = {
    ...
    createCard(person = {}, options = {}) {
      ...
      if (link) {
        ...
        element.setAttribute('aria-label', person.name ? `${person.name} profile` : 'STG member');
      }
